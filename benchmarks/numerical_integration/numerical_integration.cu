...
__global__ void _auto_kernel_0(float arr[10000],float delta)
{
  int thread_x_id;thread_x_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (thread_x_id) 
    if (thread_x_id <= 10000) {
...
/* Array to hold answer */
  float arr[10000];
/* Step size */
  float delta = ((float )10) / ((float )10000);
{
/* Auto-generated code for call to _auto_kernel_0 */
    typedef float _narray_arr;
    _narray_arr *d_arr;
    cudaMalloc((void **) &d_arr, sizeof(float ) * 10000);
    cudaMemcpy(d_arr, arr, sizeof(float ) * 10000, cudaMemcpyHostToDevice);
    int CUDA_GRID_X;
    ...
    cudaMemcpy(arr, d_arr, sizeof(float ) * 10000, cudaMemcpyDeviceToHost);
  }
/* Add up the heights of the rectangles */
  double sum = 0;
  for (i_nom_1 = 1; i_nom_1 <= 10000; i_nom_1 += 1) {
    sum += arr[1 * i_nom_1 + -1];
  }
