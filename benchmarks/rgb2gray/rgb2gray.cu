...
__global__ void _auto_kernel_0(int w,int h,unsigned char image[h][4 * w],float gray_image[h][w])
{
  ...
  if (thread_x_id && thread_y_id) 
    if (thread_x_id <= (h + 0) / 1 && thread_y_id <= (w + 3) / 4) {
      ...
      gray_image[1 * thread_x_id + -1][4 * thread_y_id + -4] = 0.21 * ((float )r) + 0.71 * ((float )g) + 0.07 * ((float )b);
    }
}
...
/* Auto-generated code for call to _auto_kernel_0 */
    ...
    typedef float _narray_gray_image[w];
    _narray_gray_image *d_gray_image;
    cudaMalloc((void **) &d_gray_image, sizeof(float ) * h * w);
    cudaMemcpy(d_gray_image, gray_image, sizeof(float ) * h * w, cudaMemcpyHostToDevice);
    int CUDA_GRID_X;
    ...
    cudaMemcpy(gray_image, d_gray_image, sizeof(float ) * h * w, cudaMemcpyDeviceToHost);
  }
/* Store the output */
